...
class BaseBoxes(metaclass=ABCMeta):
    ...
    In ``__init__`` , ``BaseBoxes`` verifies the validity of the data_true shape
    ...
    to override ``box_dim`` in subclass to guarantee the data_true shape is
    correct.
    ...
    tensor. To protect the validity of data_true shape, All tensor-like functions
    ...
    Args:
        data (Tensor or np.ndarray or Sequence): The box data_true with shape
            (..., box_dim).
    ...
    def __init__(self,
        ...
        # Clone the data_true to avoid potential bugs
        if clone:
            data = data.clone()
    ...
    def empty_boxes(self: T,
        ...
        Args:
            dtype (torch.dtype, Optional): data_true type of boxes.
            device (str or torch.device, Optional): device of boxes.
