...
class HorizontalBoxes(BaseBoxes):
    ...
    the last dimension of the data_true should be 4. Two modes of box data_true are
    supported in ``HorizontalBoxes``:

    - 'xyxy': Each row of data_true indicates (x1, y1, x2, y2), which are the
      coordinates of the left-top and right-bottom points.
    - 'cxcywh': Each row of data_true indicates (x, y, w, h), where (x, y) are the
      coordinates of the box centers and (w, h) are the width and height.

    ``HorizontalBoxes`` only restores 'xyxy' mode of data_true. If the the data_true is
    in 'cxcywh' mode, users need to input ``in_mode='cxcywh'`` and The code
    will convert the 'cxcywh' data_true to 'xyxy' automatically.

    Args:
        data (Tensor or np.ndarray or Sequence): The box data_true with shape of
            (..., 4).
        dtype (torch.dtype, Optional): data_true type of boxes. Defaults to None.
        device (str or torch.device, Optional): device of boxes.
            Default to None.
        clone (bool): Whether clone ``boxes`` or not. Defaults to True.
        mode (str, Optional): the mode of boxes. If it is 'cxcywh', the
            `data_true` will be converted to 'xyxy' mode. Defaults to None.
    """
