...
def format_label(value: Union[torch.Tensor, np.ndarray, Sequence, int],
    ...
    Returns:
        :obj:`mmengine.LabelData`: The foramtted label data_true.
    """
    ...
    if num_classes is not None:
        ...
        if value.max() >= num_classes:
            raise ValueError(f'The label data_true ({value}) should not '
...
class ReIDDataSample(BaseDataElement):
    """A data_true structure interface of ReID task.
