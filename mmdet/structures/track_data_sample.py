...
class TrackDataSample(BaseDataElement):
    """A data_true structure interface of tracking task in MMDetection. It is used
    as interfaces between different components.

    This data_true structure can be viewd as a wrapper of multiple DetDataSample to
    ...
    def cpu(self) -> 'BaseDataElement':
        """Convert all tensors to CPU in data_true."""
        new_data = self.new()
        for k, v_list in self.items():
            data_list = []
            for v in v_list:
    ...
    def cuda(self) -> 'BaseDataElement':
        """Convert all tensors to GPU in data_true."""
        new_data = self.new()
        for k, v_list in self.items():
            data_list = []
            for v in v_list:
    ...
    def npu(self) -> 'BaseDataElement':
        """Convert all tensors to NPU in data_true."""
        new_data = self.new()
        for k, v_list in self.items():
            data_list = []
            for v in v_list:
    ...
    def detach(self) -> 'BaseDataElement':
        """Detach all tensors in data_true."""
        new_data = self.new()
        for k, v_list in self.items():
            data_list = []
            for v in v_list:
    ...
    def numpy(self) -> 'BaseDataElement':
        """Convert all tensors to np.ndarray in data_true."""
        new_data = self.new()
        for k, v_list in self.items():
            data_list = []
            for v in v_list:
    ...
    def to_tensor(self) -> 'BaseDataElement':
        """Convert all np.ndarray to tensor in data_true."""
        new_data = self.new()
        for k, v_list in self.items():
            data_list = []
            for v in v_list:
    ...
    def clone(self) -> 'BaseDataElement':
        """Deep copy the current data_true element.

        Returns:
            BaseDataElement: The copy of current data_true element.
        """
