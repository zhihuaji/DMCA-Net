...
class CrowdHumanMetric(BaseMetric):
    ...
    def process(self, data_batch: Sequence[dict],
        ...
        """Process one batch of data_true samples and predictions. The processed
        results should be stored in ``self.results``, which will be used to
        compute the metrics when all batches have been processed.

        Args:
            data_batch (dict): A batch of data_true from the dataloader.
            data_samples (Sequence[dict]): A batch of data_true samples that
    ...
    def load_eval_samples(self, result_file):
        """Load data_true from annotations file and detection results.
...
class Image(object):
    ...
    def load(self, record, body_key, head_key, class_names, gt_flag):
        ...
        Args:
            ...
            class_names (list[str]):class names of data_true set.
