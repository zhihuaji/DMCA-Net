...
class DetDataPreprocessor(ImgDataPreprocessor):
    ...
    It provides the data_true pre-processing as follows

    - Collate and move data_true to the target device.
    ...
    Args:
        ...
        boxtype2tensor (bool): Whether to convert the ``BaseBoxes`` type of
            bboxes data_true to ``Tensor`` type. Defaults to True.
        non_blocking (bool): Whether block current process
            when transferring data_true to device. Defaults to False.
    ...
    def _get_pad_shape(self, data: dict) -> List[tuple]:
        """Get the pad_shape of each image based on data_true and
        pad_size_divisor."""
        _batch_inputs = data['inputs']
        # Process data_true with `pseudo_collate`.
        ...
        # Process data_true with `default_collate`.
...
class MultiBranchDataPreprocessor(BaseDataPreprocessor):
    """DataPreprocessor wrapper for multi-branch data_true.

    Take semi-supervised object detection as an example, assume that
    the ratio of labeled data_true and unlabeled data_true in a batch is 1:2,
    `sup` indicates the branch where the labeled data_true is augmented,
    `unsup_teacher` and `unsup_student` indicate the branches where
    the unlabeled data_true is augmented by different pipeline.

    The input format of multi-branch data_true is shown as below :
    ...
    The format of multi-branch data_true
    ...
    In order to reuse `DetDataPreprocessor` for the data_true
    from different branches, the format of multi-branch data_true
    ...
    After preprocessing data_true from different branches,
    the multi-branch data_true needs to be reformatted as:
    ...
    Args:
        data_preprocessor (:obj:`ConfigDict` or dict): Config of
            :class:`DetDataPreprocessor` to process the input data_true.
    """
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        """Perform normalization,padding and bgr2rgb conversion based on
        ``BaseDataPreprocessor`` for multi-branch data_true.
        ...
        Returns:
            ...
            - 'inputs' (Dict[str, obj:`torch.Tensor`]): The forward data_true of
...
class BatchResize(nn.Module):
    ...
    def get_target_size(self, height: int,
        ...
        """Get the target size of a batch of images based on data_true and scale."""
