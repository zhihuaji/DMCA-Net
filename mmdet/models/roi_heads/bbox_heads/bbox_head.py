...
class BBoxHead(BaseModule):
    ...
    def get_targets(self,
        ...
        labels, label_weights, bbox_targets, bbox_weights = multi_apply(
            ...
            pos_gt_bboxes_list,
            pos_gt_labels_list,
            cfg=rcnn_train_cfg)  # labels为proposals到底是哪一类，bbox_targets只计算非gt_bbox（神经网络预测出来的框）相对于gt的偏移量
    ...
    def loss_and_target(self,
        ...
        """ 将labels，label_weights，bbox_targets, bbox_weights 拼接到一个字典中"""
        losses = self.loss(
            cls_score,
            bbox_pred,
            rois,
            ...
            if pos_inds.any():
                ...
                if self.reg_class_agnostic:
                    pos_bbox_pred = bbox_pred.view(
                        bbox_pred.size(0), -1)[pos_inds.type(torch.bool)]  # labels为前景的bbox才计算损失
                else:
                    pos_bbox_pred = bbox_pred.view(
    ...
    def _predict_by_feat_single(
        ...
        if rcnn_test_cfg is None:
            ...
            det_labels = torch.where(det_labels >= 11, det_labels - 11, det_labels)
    ...
    InstanceList],
        ...
        labels = bbox_results['bbox_targets'][0]  # 真实值
        cls_scores = bbox_results['cls_score']  # 预测值
        rois = bbox_results['rois']  # sampling的坐标值
        bbox_preds = bbox_results['bbox_pred']  # 预测的偏移量
        if self.custom_activation:
            # TODO: Create a SeasawBBoxHead to simplified logic in BBoxHead
        ...
        if cls_scores.shape[-1] == self.num_classes + 1:
            ...
            cls_scores = cls_scores[:, :-1]  # 去除最后一列
        ...
        labels = torch.where(labels == self.num_classes, cls_scores.argmax(1),
                             labels)
        # 如果真实值labels某个元素为背景（2或12），则返回预测值cls_scores对应的列最大值，否则返回原labels数值
        ...
        for i in range(len(batch_img_metas)):
            ...
            bboxes_ = rois[inds, 1:]  # 取出同一张图片的roi坐标
            label_ = labels[inds]  # 取出同一张图片的真实label标签
            bbox_pred_ = bbox_preds[inds]  # 取出同一张图片的预测偏移量
            img_meta_ = batch_img_metas[i]  # 取出同一张图片的原始尺寸，padding，以及数据增强信息等
            pos_is_gts_ = pos_is_gts[i]  # 取出同一张图片是否为gt信息
            cls_score_ = bbox_results['cls_score'][inds]
            bboxes = self.regress_by_class(bboxes_, label_, bbox_pred_,
                                           img_meta_)  # 得到回归后的bbox
            # filter gt bboxes
            pos_keep = 1 - pos_is_gts_
            keep_inds = pos_is_gts_.new_ones(num_rois)
            keep_inds[:len(pos_is_gts_)] = pos_keep  # 将gt的index全都赋值为0，其余全赋值为1

            results = InstanceData(bboxes=bboxes[keep_inds.type(torch.bool)],
                                   scores=cls_score_[keep_inds.type(torch.bool)])
            results_list.append(results)

        return results_list  # 得到去除gt并且回归后的bbox坐标
