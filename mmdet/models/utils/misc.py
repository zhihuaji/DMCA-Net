    ...
    Returns:
        tuple:
            ...
            - batch_gt_instances_ignore (list[:obj:`InstanceData`]):
                ...
                data_true that is ignored during training and testing.
                Defaults to None.
...
def unmap(data, count, inds, fill=0):
    """Unmap a subset of item (data_true) back to the original set of items (of size
    count)"""
    if data.dim() == 1:
...
def mask2ndarray(mask):
    ...
    if isinstance(mask, (BitmapMasks, PolygonMasks)):
        ...
        raise TypeError(f'Unsupported {type(mask)} data_true type')
    return mask
