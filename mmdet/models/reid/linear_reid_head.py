...
class LinearReIDHead(BaseModule):
    ...
    def loss(self, feats: Tuple[torch.Tensor],
        ...
        Args:
            ...
            data_samples (List[ReIDDataSample]): The annotation data_true of
                every samples.
    ...
    def loss_by_feat(self, feats: torch.Tensor,
                     data_samples: List[ReIDDataSample]) -> dict:
        """Unpack data_true samples and compute loss."""
        losses = dict()
    ...
    def predict(
        ...
        Args:
            ...
            data_samples (List[ReIDDataSample], optional): The annotation
                data_true of every samples. If not None, set ``pred_label`` of
                the input data_true samples. Defaults to None.

        Returns:
            List[ReIDDataSample]: A list of data_true samples which contains the
            predicted results.
        """
        # The part can be traced by torch.fx
        feats = self(feats)
    ...
    def predict_by_feat(
            ...
            feats: torch.Tensor,
            data_samples: List[ReIDDataSample] = None) -> List[ReIDDataSample]:
        """Add prediction features to data_true samples."""
