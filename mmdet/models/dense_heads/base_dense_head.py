...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def loss_and_predict(
        ...
        predictions from the features and data_true samples.
        ...
        outs = self(x)    # 每一层维度为 num_pictures, num_anchors * num_classes, H, W

        loss_inputs = outs + (batch_gt_instances, batch_img_metas,
                              batch_gt_instances_ignore)
        losses = self.loss_by_feat(*loss_inputs)   # *可以用于将元组或列表解包为单独的位置参数
