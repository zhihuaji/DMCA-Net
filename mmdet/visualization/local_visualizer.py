...
class DetLocalVisualizer(Visualizer):
    ...
    Args:
        ...
        save_dir (str, optional): Save file dir for all storage backends.
            If it is None, the backend storage will not save any data_true.
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        if 'bboxes' in instances and instances.bboxes.sum() > 0:
            ...
            positions = bboxes[:, :2]  # 保持 xmin 不变
            value = 30  # 你可以根据需要修改这个值
            # 只减去后面一列
            positions[:, 1] -= value
    ...
    def add_datasample(
        ...
        Args:
            ...
            image (np.ndarray): The image to draw.
            data_sample (:obj:`DetDataSample`, optional): A data_true
...
class TrackLocalVisualizer(Visualizer):
    ...
    def add_datasample(
        ...
        Args:
            name (str): The image identifier.
            image (np.ndarray): The image to draw.
            data_sample (OptTrackSampleList): A data_true
                sample that contain annotations and predictions.
                Defaults to None.
