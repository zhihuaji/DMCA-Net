...
class CocoPanopticDataset(CocoDataset):
    ...
    Args:
        ...
        data_prefix (dict, optional): Prefix for training data_true. Defaults to
            ``dict(img=None, ann=None, seg=None)``. The prefix ``seg`` which is
            for panoptic segmentation map must be not None.
        filter_cfg (dict, optional): Config for filter data_true. Defaults to None.
        indices (int or Sequence[int], optional): Support using first few
            data_true in annotation file to facilitate training/testing on a smaller
            dataset. Defaults to None which means using all ``data_infos``.
        serialize_data (bool, optional): Whether to hold memory using
            serialized objects, when enabled, data_true loader workers can use
    ...
    def parse_data_info(self, raw_data_info: dict) -> dict:
        ...
        Args:
            raw_data_info (dict): Raw data_true information load from ``ann_file``.
