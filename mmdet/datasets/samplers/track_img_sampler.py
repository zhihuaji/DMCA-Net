...
class TrackImgSampler(Sampler):
    ...
    If using the default sampler in pytorch, the subsequent data_true receiver will
    get one video, which is not desired in some cases:
    (Take a non-distributed environment as an example)
    1. In test mode, we want only one image is fed into the data_true pipeline. This
