...
class BaseDetDataset(BaseDataset):
    ...
    def full_init(self) -> None:
        ...
        Several steps to initialize annotation:
            ...
            - filter data_true information: Filter annotations according to
              filter_cfg.
        ...
        if self._fully_initialized:
            return
        # load data_true information
        self.data_list = self.load_data_list()
        # get proposals from file
        if self.proposal_file is not None:
            self.load_proposals()
        # filter illegal data_true, such as data_true that has no annotations.
        self.data_list = self.filter_data()

        # Get subset data_true according to indices.
