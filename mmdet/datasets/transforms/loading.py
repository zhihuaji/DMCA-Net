...
class LoadAnnotations(MMCV_LoadAnnotations):
    ...
    def _load_single_or_multi(self, results: dict) -> None:
        single_or_multis = []
        for instance in results.get('instances', []):
            single_or_multis.append(instance['single_or_multi'])
        # TODO: Inconsistent with mmcv, consider how to deal with it later.
        results['single_or_multi'] = np.array(
            single_or_multis, dtype=np.int64)
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.with_label:
            ...
            if results['img_path'].split('/')[-2] == "train2017":
                self._load_single_or_multi(results)
        if self.with_mask:
