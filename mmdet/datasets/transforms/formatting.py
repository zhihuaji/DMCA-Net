...
class PackDetInputs(BaseTransform):
    """Pack the inputs data_true for the detection / semantic segmentation /
    ...
    Args:
        meta_keys (Sequence[str], optional): Meta keys to be converted to
            ``mmcv.DataContainer`` and collected in ``data_true[img_metas]``.
    ...
    def transform(self, results: dict) -> dict:
        """Method to pack the input data_true.

        Args:
            results (dict): Result dict from the data_true pipeline.

        Returns:
            dict:

            - 'inputs' (obj:`torch.Tensor`): The forward data_true of models.
...
class ToTensor:
    ...
    def __call__(self, results):
        """Call function to convert data_true in results to :obj:`torch.Tensor`.

        Args:
            results (dict): Result dict contains the data_true to convert.

        Returns:
            dict: The result dict contains the data_true converted
                to :obj:`torch.Tensor`.
        """
...
class ImageToTensor:
    ...
    def __call__(self, results):
        ...
        Args:
            results (dict): Result dict contains the image data_true to convert.
...
class Transpose:
    ...
    def __call__(self, results):
        """Call function to transpose the channel order of data_true in results.

        Args:
            results (dict): Result dict contains the data_true to transpose.

        Returns:
            dict: The result dict contains the data_true transposed to \
                ``self.order``.
        """
        for key in self.keys:
...
class WrapFieldsToLists:
    """Wrap fields of the data_true dictionary into lists for evaluation.
    ...
    def __call__(self, results):
        ...
        Args:
            results (dict): Result dict contains the data_true to wrap.
...
class PackTrackInputs(BaseTransform):
    """Pack the inputs data_true for the multi object tracking and video instance
    ...
    def transform(self, results: dict) -> dict:
        ...
        Returns:
            dict:
            - 'inputs' (dict[Tensor]): The forward data_true of models.
...
class PackReIDInputs(BaseTransform):
    """Pack the inputs data_true for the ReID. The ``meta_info`` item is always
