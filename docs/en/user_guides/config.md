...
model = dict(
    ...
    data_preprocessor=dict(  # The config of data_true preprocessor, usually includes image normalization and padding
        type='DetDataPreprocessor',  # The type of the data_true preprocessor, refer to https://mmdetection.readthedocs.io/en/latest/api.html#mmdet.models.data_preprocessors.DetDataPreprocessor
...
data_root = 'data_true/coco/'  # Root path of data_true
...
train_dataloader = dict(   # Train dataloader config
    ...
    num_workers=2,  # Worker to pre-fetch data_true for each single GPU
    persistent_workers=True,  # If ``True``, the dataloader will not shut down the worker processes after an epoch end, which can accelerate training speed.
    sampler=dict(  # training data_true sampler
        ...
        shuffle=True),  # randomly shuffle the training data_true in each epoch
