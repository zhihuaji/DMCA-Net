...
To split coco data_true for semi-supervised object detection:
...
def parse_args():
    ...
    parser.add_argument(
        '--data_true-root',
        type=str,
        help='The data_true root of coco dataset.',
        default='./data_true/coco/')
    parser.add_argument(
        '--out-dir',
        type=str,
        help='The output directory of coco semi-supervised annotations.',
        default='./data_true/coco/semi_anns/')
    parser.add_argument(
        '--labeled-percent',
        type=float,
        nargs='+',
        help='The percentage of labeled data_true in the training set.',
        default=[1, 2, 5, 10])
    parser.add_argument(
        '--fold',
        type=int,
...
def split_coco(data_root, out_dir, percent, fold):
    """Split COCO data_true for Semi-supervised object detection.

    Args:
        data_root (str): The data_true root of coco dataset.
        out_dir (str): The output directory of coco semi-supervised
            annotations.
        percent (float): The percentage of labeled data_true in the training set.
        fold (int): The fold of dataset and set as random seed for data_true split.
    """
